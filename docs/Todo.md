...
- [x] 23.實作 LINE Bot Quick Reply 功能 2025-08-16
- [ ] 24.實作 LINE Bot Rich Menu 六格配置與點擊事件處理
...
- [ ] 25.擴展 User 模型支援住家與公司地點設定功能
  - **目的**：為用戶提供個人化的常用地點設定，支援「查住家」和「查公司」快捷功能，是 Rich Menu 核心功能的資料基礎。
  - **為何重要**：這是 PRD 中最高頻使用功能的實現基礎。用戶設定住家和公司地址後，可透過 Rich Menu 一鍵查詢，大幅提升使用體驗和效率。沒有此功能，Rich Menu 的主要價值無法體現。
  - **可能的挑戰**：
    - 資料庫遷移需要謹慎處理外鍵約束和既有資料的相容性。
    - 地點設定的 UX 流程設計，需要整合地點解析功能。
    - 用戶可能輸入不存在的地址，需要驗證機制。
    - 地點變更的歷史記錄和版本管理考量。
  - **實作方向與細節**：
    1. 修改 `app/user/models.py` 的 `User` 模型，新增 `home_location_id` 和 `work_location_id` 欄位，建立與 `Location` 的外鍵關聯。
    2. 執行 Alembic 遷移，新增相關欄位和約束。
    3. 更新 `app/user/schemas.py` 中的 Pydantic schema，支援地點設定的輸入和輸出。
    4. 在 `app/user/service.py` 中新增 `set_user_location` 和 `get_user_locations` 方法。
    5. 設計地點設定的對話流程，整合既有的地點解析功能。
    6. 在 `app/line/service.py` 中實作「我的地點」功能的對話邏輯。
    7. 撰寫完整的測試案例覆蓋地點設定、查詢和驗證邏輯。
  - **預期成果與驗收依據**：
    - 用戶可透過 LINE Bot 設定和修改住家、公司地址。
    - 設定的地點會經過地點解析驗證，確保地址的有效性。
    - Rich Menu 的「查住家」、「查公司」功能可正確讀取用戶設定。
    - 資料庫正確儲存用戶地點設定，並支援 CRUD 操作。
- [ ] 26.實作用戶查詢記錄與最近查詢功能
