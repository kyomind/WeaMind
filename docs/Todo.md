...
- [ ] 14.執行 Alembic 資料庫遷移，建立 user table
  ...
  - **實作方向與細節**：
    1. 確認 `app/user/models.py` 中的 `User` SQLAlchemy 模型已最終確定，並包含上述欄位。
    ...
    4. 檢查並修改產生的遷移腳本，確保它包含了建立 `user` 表格的 `op.create_table()` 指令，以及對應的欄位和約束。
    5. 執行 `alembic upgrade head` 以套用遷移。
    6. 使用資料庫客戶端工具連接到開發資料庫，驗證 `user` 表格已按預期建立。
