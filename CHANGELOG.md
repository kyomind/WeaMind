...
### 修正
- **天氣查詢一致性與穩定性**: 修正同一次查詢可能重複判定地點的問題，確保天氣結果與最近查詢記錄一致，並降低高併發時資料庫連線占用。
- **相依套件安全性更新**: 更新具已知安全風險的相依套件，提升服務整體安全性與穩定性。

## [1.2.4] - 2026-07-29

### 修正
- **Redis 安全性強化**: 升級 Redis 至安全修補版本，並限制僅能透過內部網路存取，降低未授權連線與服務遭利用的風險。
- **相依套件安全性更新**: 更新具已知安全風險的相依套件，提升服務整體安全性與穩定性。

## [1.2.3] - 2026-07-06

### 修正
- **相依套件安全性更新**: 更新多個關鍵相依套件的安全版本，降低已知漏洞風險並提升服務穩定性。

### 改進
- **映像版本標籤更清晰**: GHCR 發布流程改為保留 `latest` 與語意化版本 tags，例如 `1.2.3`、`1.2`、`1`，減少頻繁更新造成的 `sha-*` 標籤干擾。

## [1.2.2] - 2026-05-01

### 新增
- **Release 後自動更新 Infra**: 每次 WeaMind 發布新版本時，會自動對 weamind-infra 開出版本更新 PR，讓正式部署版本跟著 release 保持同步。

## [1.2.1] - 2026-04-25

### 修正
- **🔒 相依套件安全性更新**: 更新 Mako 等關鍵相依套件安全版本，降低已知漏洞帶來的風險並提升服務穩定性

## [1.2.0] - 2026-04-24

### 新增
- **📊 Webhook 監控指標**: 新增 LINE webhook 處理指標與 metrics 端點，讓服務狀態、成功率與處理延遲更容易被觀察與追蹤

### 修正
- **🛡️ 相依套件安全性更新**: 更新 aiohttp 與其他關鍵套件的安全版本，降低已知漏洞帶來的風險

### 改進
- **⚙️ Webhook 事件處理可觀測性提升**: 將 LINE webhook 的成功、失敗與處理時間統計細化到事件層級，提升問題排查效率與服務穩定性

## [1.1.2] - 2026-03-17

### 修正
- **🔒 安全漏洞修復**: 升級 cryptography 套件至 46.0.5，修復 CVE-2026-26007 安全漏洞，強化系統資料傳輸保護

### 改進
- **🐍 Python 執行環境升級**: 升級至 Python 3.12.12，提升系統穩定性與執行效能

## [1.1.1] - 2026-01-19

### 修正
- **🔒 安全性更新**: 修復 virtualenv 套件安全漏洞 (GHSA-597g-3phw-6986)，確保系統安全性

### 改進
- **🐳 Docker 部署策略優化**: 調整容器映像標籤策略，提升版本管理明確性與部署可靠度

## [1.1.0] - 2026-01-12

### 新增
- **🚀 GitHub Container Registry 自動化發布**: 建置完整的 GHCR 自動發布工作流程，每次版本發布時自動建置並推送 Docker 映像，大幅簡化部署流程
- **🌐 多平台映像支援**: 新增 amd64 和 arm64 雙平台支援，讓 WeaMind 可在更多種類的伺服器和雲端環境上運行，提升部署彈性
- **🏷️ 智慧版本標籤管理**: 自動為 Docker 映像建立 latest 和版本號標籤，方便追蹤不同版本的部署狀態
- **☸️ Kubernetes 就緒部署架構**: 完善容器化基礎設施，結合健康檢查端點與自動化映像發布，為未來 Kubernetes 編排部署奠定堅實基礎

## [1.0.6] - 2026-01-12

### 新增
- **🔧 系統健康檢查**: 新增健康檢查端點，為未來容器化部署（Kubernetes）做好準備，提升服務穩定性

### 修正
- **🔒 安全性更新**: 修復依賴套件安全漏洞，確保系統安全性

## [1.0.5] - 2025-10-08

### 改進
- **📖 技術文件改善**: 重新整理開發者技術亮點說明，讓專案的技術特色更加清晰易懂

## [1.0.4] - 2025-10-07

### 新增
- **📖 新增專案文件開發者部分**: 新增技術架構說明與開發者指南，讓專案資訊更完整易懂

## [1.0.3] - 2025-10-02

### 新增
- **🔒 資安防護機制**: 新增 detect-secrets、bandit 和 safety 等安全性掃描工具，自動檢測並防止敏感資訊洩露，提升系統安全性

## [1.0.2] - 2025-09-30

### 修正
- **📊 查詢記錄完整性**: 修正「查住家」和「查公司」功能未記錄查詢歷史的問題，確保使用記錄完整準確

## [1.0.1] - 2025-09-30

### 改進
- **🧹 資料架構最佳化**: 精簡用戶資料結構，提升系統效能與維護性

## [1.0.0] - 2025-09-30

### 🎉 正式版本發布
WeaMind 智慧天氣 LINE Bot 正式推出 1.0.0 版本！經過長期開發與優化，現已具備完整的天氣查詢功能，提供台灣用戶快速、準確的天氣資訊服務。

### 改進
- **📖 使用說明完善**: 新增詳細的功能介紹與操作指南，讓新用戶能快速上手各項功能
- **🎨 介面視覺優化**: 調整功能描述文字與版面配置，提升整體使用體驗
- **🔗 快速存取連結**: 優化功能說明頁面的導航連結，方便用戶快速查閱使用方式

### 🌟 主要功能特色
- **🌤️ 智慧天氣查詢**: 支援文字搜尋、地圖選點、快速重複查詢
- **🏠 個人化設定**: 住家與公司地點設定，一鍵查詢常用地址天氣
- **📱 直觀操作介面**: LINE Bot Rich Menu 設計，所有功能一目了然
- **⚡ 即時資料更新**: 提供最新的天氣預報資訊，確保查詢結果準確可靠

## [0.11.1] - 2025-09-28

### 改進
- **🧹 系統架構精簡**: 移除公告系統功能，優化選單結構，讓 LINE Bot 介面更簡潔易用

## [0.11.0] - 2025-09-26

### 新增
- **🌤️ 天氣資料新鮮度檢查**: 新增智慧資料時效性檢查機制，確保用戶獲得最新準確的天氣預報資訊，提升查詢結果可靠性
- **⏰ 資料更新時間戳**: 在天氣回應中顯示資料最後更新時間，讓用戶清楚了解天氣資訊的時效性

### 改進
- **🎨 介面顯示最佳化**: 優化時間顯示格式，移除不必要的視覺元素，讓天氣資訊呈現更簡潔明瞭

## [0.10.2] - 2025-09-25

### 改進
- **🧪 測試程式碼重構**: 消除測試程式碼重複，提升開發效率與程式碼品質，間接確保服務更加穩定可靠

## [0.10.1] - 2025-09-25

### 改進
- **🔧 LINE Bot 架構優化**: 重構訊息處理和 PostBack 回應邏輯，提升系統穩定性和程式碼維護性

## [0.10.0] - 2025-09-24

### 新增
- **🌤️ 天氣資料查詢功能**: 正式啟用從資料庫獲取並回傳實際天氣預報的核心功能，WeaMind 現在可以提供真實的天氣資訊服務

## [0.9.1] - 2025-09-14

### 改進
- **🛡️ 分散式鎖機制優化**: 改用固定時間 TTL，簡化鎖定邏輯
- **💬 操作體驗優化**: 更新系統提示訊息，當操作過於頻繁時顯示更友善的引導文字

## [0.9.0] - 2025-09-14

### 改進
- **⚡ 快速 Webhook 回應機制**: 導入 Fast ACK 架構，立即回應 LINE 伺服器請求，有效解決併發時段的訊息處理問題
- **🚀 Webhook 架構優化**: 將處理邏輯從同步改為背景執行，避免處理時間影響 LINE 伺服器回應，有效減少 redelivery 問題
